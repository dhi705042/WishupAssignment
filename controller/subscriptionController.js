...
const registerUserSubscription = async function (req, res) {
    try {
    ...
    try {
        ...
        for (let i = 0; i < userSubscription.length; i++) {
            ...
            if (userSubscription[i].planId == 'FREE') {
                ...
                let expiryDateFormat = moment(expiry).format('YYYY-MM-DD')

                let obj = { plan_id: userSubscription[i].planId, start_date: userSubscription[i].startDate, valid_till: expiryDateFormat }
                userDetails.push(obj)
            }
        }
